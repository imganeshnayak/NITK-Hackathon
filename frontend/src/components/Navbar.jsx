    ...
    <nav className="relative bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm shadow-lg border-b border-gray-200 dark:border-gray-700 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="sm:hidden flex items-center">
            {/* The toggles are REMOVED from here for a cleaner look */}
      ...
      {isMenuOpen && (
        <div className="sm:hidden absolute top-16 left-0 w-full bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm shadow-lg px-2 pt-2 pb-3 space-y-1">
          {/* Main Links */}
          {token ? (
            <button onClick={handleLogout} className="w-full text-left block px-3 py-2 rounded-md text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700">
              Logout
            </button>
          ) : (
            <>
          ...
          {/* --- Toggles are now inside the dropdown on mobile --- */}
